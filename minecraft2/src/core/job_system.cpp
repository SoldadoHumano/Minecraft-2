...
void JobSystem::Initialize() {
    ...
    // We reserve one thread for the OS Main loop and one for the Vulkan Render thread.
    // So we use max(2, hardware_threads - 2) for our worker pool.
    uint32_t workerCount = std::max(2u, numThreads > 2 ? numThreads - 2 : 2);
    
    std::cout << "[JobSystem] Initializing with " << workerCount << " worker threads.\n";
