...
public:
  ...
  void SetMeshReadyCallback(std::function<void(const ChunkMesh &)> callback) {
    m_meshReadyCallback = callback;
  }
  void SetChunkUnloadCallback(std::function<void(int, int)> callback) {
...
private:
  ...
  enum class ChunkState { LOADING, GENERATED, MESHED };
