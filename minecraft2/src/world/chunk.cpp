...
void Chunk::Generate(const mc::math::PerlinNoise& noise) {
    ...
    alignas(32) float noiseVal_arr[CHUNK_WIDTH * CHUNK_DEPTH];
    ...
    for (int i = 0; i < totalBlocks; i += 8) {
        ...
        _mm256_storeu_ps(&noiseVal_arr[i], vNoise);
    }
    
    for (int x = 0; x < CHUNK_WIDTH; ++x) {
        for (int z = 0; z < CHUNK_DEPTH; ++z) {
            float noiseVal = noiseVal_arr[x * CHUNK_DEPTH + z];
            int height = 64 + static_cast<int>(noiseVal * 32.0f);
...
void Chunk::SetNeighbors(std::shared_ptr<Chunk> nxp, std::shared_ptr<Chunk> nxm, std::shared_ptr<Chunk> nzp, std::shared_ptr<Chunk> nzm) {
    m_neighborXP = nxp;
    m_neighborXM = nxm;
    m_neighborZP = nzp;
    m_neighborZM = nzm;
...
bool Chunk::IsFaceVisible(int x, int y, int z) const {
    if (y < 0 || y >= CHUNK_HEIGHT) return true; // Always draw top/bottom borders

    if (x < 0) {
        if (m_neighborXM) return m_neighborXM->GetBlock(CHUNK_WIDTH - 1, y, z) == BlockType::Air;
        return true;
    }
    if (x >= CHUNK_WIDTH) {
        if (m_neighborXP) return m_neighborXP->GetBlock(0, y, z) == BlockType::Air;
        return true;
    }
    if (z < 0) {
        if (m_neighborZM) return m_neighborZM->GetBlock(x, y, CHUNK_DEPTH - 1) == BlockType::Air;
        return true;
    }
    if (z >= CHUNK_DEPTH) {
        if (m_neighborZP) return m_neighborZP->GetBlock(x, y, 0) == BlockType::Air;
        return true;
    }
...
void Chunk::AddGreedyQuad(std::vector<mc::render::Vertex>& vertices, std::vector<uint32_t>& indices, 
                          int x, int y, int z, int face, BlockType type, int wx, int wy, int wz) {
...
struct MaskBlock { bool visible; BlockType type; };
...
void Chunk::BuildMeshes(std::vector<struct ChunkMesh>& outMeshes) {
    ...
    for (int sectionY = 0; sectionY < 16; ++sectionY) {
            ...
            for (int slice = 0; slice < 16; ++slice) {
                ...
                for (int u = 0; u < 16; ++u) {
                    for (int v = 0; v < 16; ++v) {
                        int x = 0, y = 0, z = 0;
                        int nx = 0, ny = 0, nz = 0;
                        if (face == 0) { x = slice; y = startY + u; z = v; nx = x + 1; ny = y; nz = z; }
                        else if (face == 1) { x = slice; y = startY + u; z = v; nx = x - 1; ny = y; nz = z; }
                        else if (face == 2) { y = startY + slice; x = u; z = v; nx = x; ny = y + 1; nz = z; }
                        else if (face == 3) { y = startY + slice; x = u; z = v; nx = x; ny = y - 1; nz = z; }
                        else if (face == 4) { z = slice; x = u; y = startY + v; nx = x; ny = y; nz = z + 1; }
                        else if (face == 5) { z = slice; x = u; y = startY + v; nx = x; ny = y; nz = z - 1; }
                ...
                for (int u = 0; u < 16; ++u) {
                    for (int v = 0; v < 16; ++v) {
                        if (mask[u][v].visible) {
                            ...
                            while (v + w < 16 && mask[u][v + w].visible && mask[u][v + w].type == type) {
                                w++;
                            }
                            ...
                            while (u + h < 16) {
                                for (int k = 0; k < w; ++k) {
                                    if (!mask[u + h][v + k].visible || mask[u + h][v + k].type != type) {
                                        done = true;
                                        break;
                                    }
                                }
                            ...
                            int x = 0, y = 0, z = 0;
                            if (face == 0 || face == 1) { x = slice; y = u + startY; z = v; }
                            else if (face == 2 || face == 3) { x = u; y = slice + startY; z = v; }
                            else if (face == 4 || face == 5) { x = u; y = v + startY; z = slice; }
                            
                            int wx = 1, wy = 1, wz = 1;
                            if (face == 0 || face == 1) { wy = h; wz = w; }
                            else if (face == 2 || face == 3) { wx = h; wz = w; }
                            else if (face == 4 || face == 5) { wx = h; wy = w; }
        ...
        if (indices.size() > indicesBefore) {
            if (startY < meshMinY) meshMinY = startY;
            if (startY + 16 > meshMaxY) meshMaxY = startY + 16;
        }
    }
